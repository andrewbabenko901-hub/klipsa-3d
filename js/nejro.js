  ...
  nvidia: {
    ...
    async spisokModeley(klyuch, adres) {
      ...
      if (!r.ok) {
        ...
        if (r.status === 403 || r.status === 401) throw new Error(OTKAZ_403);
    ...
    async razobrat(klyuch, model, foto, promt, adres) {
      ...
      try {
        ...
        if (/\b40[13]\b|Authorization failed|Unauthorized/i.test(e.message)) throw new Error(OTKAZ_403);
        if (!/nvext|guided|400|422/i.test(e.message)) throw e;
        // сюда доходят только 400/422 про nvext — остальное уже отсеяно выше
...
const OTKAZ_403 =
  'NVIDIA не приняла ключ (401 Unauthorized или 403 Authorization failed — она отвечает ' +
  'и так, и так). Шлюз при этом работает — значит ' +
