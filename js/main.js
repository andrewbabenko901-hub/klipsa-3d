...
async function sintez() {
  ...
  try {
    ...
    if (S.listPozzhe) { listNeRisovali(S.listPozzhe); S.listPozzhe = null; }
    ...
    if ($('#chList').checked) {
      ...
      if (!gk) { shag('list','sboj'); skazatOshibku('Для листа картинкой нужен ключ: ' +
        ...
        listNeRisovali('Выбранная рисовалка — «' + LS.modelKartinki + '» — чистая диффузионка: ' +
          'фотографию она не видит и нарисует не твою клипсу, а похожую железку. Поэтому лист ' +
          'ей не заказывался. Четыре вида твоей детали уже построены из настоящей геометрии — ' +
          'спереди, сбоку, сверху и в изометрии, они на вкладке «Лист разбора».');
        shag('list', null);   // серый: шаг не делали, а не выполнили
      }
 ...
 *
 * Показывать здесь копию «Листа разбора» нельзя: две вкладки с одной и той же
 * картинкой — обман, человек решит, что нейронка что-то нарисовала. Пишем прямо,
 * что не рисовала и почему, и даём кнопку туда, где четыре вида уже есть.
 */
function listNeRisovali(pochemu) {
  $('#nejroList').innerHTML =
    `<div style="padding:14px;max-width:620px">
       <div style="font-weight:600;margin-bottom:6px">Нейронка лист не рисовала</div>
       <div class="podskazka" style="margin-bottom:10px">${pochemu}</div>
       <button class="knopka" id="knKListuRazbora">Открыть «Лист разбора» — четыре вида</button>
     </div>`;
  const kn = $('#knKListuRazbora');
  if (kn) kn.onclick = () => vkladka('razbor');
}
...
function vkladka(v) {
  ...
  const t = { model:'ЛКМ — вращение · колесо — зум · ПКМ — сдвиг',
    ...
    // Подпись под вкладкой не должна спорить с тем, что на ней написано:
    // если мы уже объяснили, почему нейронка не рисовала, второй раз не зовём.
    nejro: (S.nejro || ($('#nejroList')?.textContent || '').trim()) ? ''
      : 'Лист нейронкой не запрашивался — поставь галочку и синтезируй заново',
